#!/usr/bin/env python3
"""
Cross-platform script to run transformer training.
...
def run_training():
    ...
    print_header("Starting Transformer Training Pipeline")
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "src.models.transformer_training"],
            check=False
        )
        
        print("\n" + "=" * 60)
        if result.returncode == 0:
            print_success("✓ Transformer Training Complete!")
            print_success("=" * 60)
            print("\nModel saved to: models/transformer/distilbert/")
            print("\nNext steps:")
            print("  1. Run tests: python run_tests.py")
            print("  2. Check results: python tests/test_metrics.py")
            return 0
        else:
