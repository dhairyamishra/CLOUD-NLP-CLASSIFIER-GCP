...
│                    USER INTERFACES                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ REST API     │  │ Streamlit UI │  │ CLI Scripts  │      │
│  │ (FastAPI)    │  │ (Interactive)│  │ (Training)   │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    4 MODELS (Multi-Model System)            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ DistilBERT   │  │ Logistic Reg │  │ Linear SVM   │      │
│  │ 96.57% acc   │  │ 85-88% acc   │  │ 85-88% acc   │      │
│  │ ~8-20ms      │  │ ~0.66ms      │  │ ~0.60ms      │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
│  ┌──────────────┐                                           │
│  │ Toxicity     │  Multi-label (6 categories)               │
│  │ Multi-Head   │  toxic, severe_toxic, obscene, etc.       │
│  └──────────────┘                                           │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    DATA PIPELINE                            │
│  Raw CSV → Preprocessing → Train/Val/Test Split             │
...
- **Transformer Model**: Fine-tuned DistilBERT (96.57% accuracy, 17-20ms inference)
...
- **Container**: Docker with multi-model support (DistilBERT, Logistic Regression, Linear SVM, Toxicity)
- **Deployment Method**: Automated via `gcp-complete-deployment.ps1`

**Live Endpoints:**
- 🏥 Health Check: http://35.232.76.140:8000/health
- 🔮 Predictions: http://35.232.76.140:8000/predict
- 📚 API Docs: http://35.232.76.140:8000/docs
- 🔄 Model Switching: http://35.232.76.140:8000/models

**Performance (Cloud VM):**
- DistilBERT: ~60-100ms avg latency
- Logistic Regression: ~5ms avg (21x faster)
- Linear SVM: ~2ms avg (44x faster)

**Cost**: ~$0.07/hour when running (~$50/month)
